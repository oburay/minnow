...
int main()
{
  ...
  datagram += string( 7, 0 );     // Setting null values for the next 7 bytes for headers : Type of Service,Total
                                  // Length, Identification, Flags,  Fragment Offset.
  datagram += char( 100 );        // Byte for TTL header
  datagram += char( 5 ); // Bytes for protocol header. Note for a user datagram, this changes to "17" per RFC791's
