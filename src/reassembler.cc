...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
 ...
 if((first_index < next_index)){
  first_index = next_index -first_index;      // Calculate the overlaping index within overlapping data stream
    ...
    if((first_index >= data.size())){         // Address edge cases where overlapping index is out of bound for data stream
        return;
      }
...
data.resize(min(data.size(), ( output_.writer().available_capacity())));      // Just resize the input data to fit our available capacity
...
if ((first_index - next_index) < output_.writer().available_capacity()) {   
    ...
    reassembler_buffer.resize(output_.writer().available_capacity(), '0');       // Resize the reassembler's buffer to the available capacity of byte stream & insert 0 for the new spaces
    reassembler_track.resize(output_.writer().available_capacity(), '0');        // Resize the tracker as well with 0s to indicate those index haven't been filled
...
if (track_count > 0) {
  ...
  reassembler_buffer = reassembler_buffer.substr(track_count);          // Clear the part going to be pushed to the bystream
  reassembler_track  = reassembler_track.substr(track_count);           // Clear the part going to be pushed to the bystream         
 ...
 // Need to write data that needs to be popped from the reassembler 
 // 
