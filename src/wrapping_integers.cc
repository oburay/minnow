...
uint64_t Wrap32::unwrap( Wrap32 zero_point, uint64_t checkpoint ) const
{
  ...
  uint64_t diff1 = (lower_candidate < checkpoint ? (checkpoint - lower_candidate) : (lower_candidate - checkpoint)) ;
  uint64_t diff2= ( mid_candidate < checkpoint ? (checkpoint - mid_candidate) : (mid_candidate - checkpoint)) ;
  uint64_t diff3 = ( upper_candidate < checkpoint ? (checkpoint - upper_candidate) : (upper_candidate - checkpoint) ) ;

  // Return candidate with lowest distance from checkpoint
  return (min(diff1,min(diff2,diff3)) == diff1 ? lower_candidate : min(diff1,min(diff2,diff3)) == diff2 ? mid_candidate : upper_candidate );
